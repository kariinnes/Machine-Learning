...
I tried several methods to model the training set but found that the **rpart** method had very low accuracy and the **treebag (bagging)** method .....  The model with the best accuracy (although with a much longer processing time) was the random forest model. I will use a random forest model with a 5-fold cross validation.   
...
modelFit <- train(training$classe ~ ., data=training, method="treebag", trControl = trainControl(method = "cv", number = 5))
print(modelFit$finalModel)
...
modelFit <- train(training$classe ~ ., data=training, method="gbm", verbose=FALSE, trControl = trainControl(method = "cv", number = 5))
print(modelFit$finalModel)
...
##Out of Sample Error
The out of sample error is the error rate you get when you run your predicted model on a new data set.  Since we created a validation set, we can run our model against the validation set to check accuracy.  We can then use these result to further refine our model prior to running it **one time only** on the test dataset.
